...
- 同位素计算：处理稳定同位素、溶质浓度、检测限和质量守恒约束，反演多个补给端元比例。支持每个观测与端元携带逐指标 1σ 标准差或完整 3×3 协方差（含端元协方差随比例传播），以加权最小二乘（`C(f)=C_obs+Σf_j²C_j`）在单纯形约束（比例非负、总和为一）上用 BB 投影梯度求解；返回逐指标加权残差、χ²、收敛原因、有效秩/条件数与不可辨识提示。相同输入与模型版本生成同一任务键，求解只依据入队时固化的原始观测与权重快照，便于复算。
