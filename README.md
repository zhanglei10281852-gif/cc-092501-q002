...
- 同位素计算：处理稳定同位素、溶质浓度、检测限和质量守恒约束，反演多个补给端元比例。
  反演为约束加权最小二乘：观测与端元均可携带逐指标标准差或 3×3 协方差，端元不确定度按
  `C(f)=C_obs+Σf_j²·C_j` 传播；比例严格满足非负与总和为一，结果返回白化加权残差、χ²、
  收敛原因、比例近似标准误和可辨识性诊断（观测不足、端元特征共线等）。相同输入与模型版本
  生成同一任务键，比例行按端元编号有序返回并带确定性名次；任务快照保留原始观测、解析后的
  权重及其来源、求解器与模型摘要，支持通过 `GET /api/hydro/inversions/{id}` 复算结论。
